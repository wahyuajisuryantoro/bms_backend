...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            // Simpan data + reverse lookup email -> token
            Cache::put('pending_user_'.$verificationToken, $userData, now()->addHours(24));
            Cache::put('pending_email_'.$request->email, $verificationToken, now()->addHours(24));
    ...
    public function resendVerification(Request $request)
    {
        ...
        // Hapus yang lama
        Cache::forget('pending_user_'.$oldToken);

        // Simpan dengan token baru
        Cache::put('pending_user_'.$newToken, $data, now()->addHours(24));
        Cache::put('pending_email_'.$request->email, $newToken, now()->addHours(24));
